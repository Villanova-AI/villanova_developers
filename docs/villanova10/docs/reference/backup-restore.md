...
# Backup and Restore an Villanova Application

This guide provides general instructions on how to backup and restore an existing Villanova Application. The process entails the migration of databases, filesystem data, Keycloak instance, and bundles, plus the reconfiguration of the service address. 

### Initial Considerations to Note
* The restored instance can be on another cluster or a different namespace of the same cluster hosting the original Villanova instance. 
...
* Bundles are deployed via Kubernetes to a specific Villanova Application and therefore must be reinstalled after the new instance is built. More details are below.

## Copy and Migrate Databases

1. Make a copy of the source databases or schemas using the native DBMS tools. Some standard Villanova database schemas to back up are `prod_dedb`, `prod_kc_db`, `prod_portdb`, and `prod_servdb`, in addition to any that were created for your microservices. Those will follow a similar naming convention, with the microservice name followed by the suffix `_plugindb`.
      * If the original database setup used a **custom resource**, refer to the [Database Custom Resource](./database-cr.md) document. 
      * If it was provisioned using **environment variables** (PORTDB_*, SERVDB_* variables) in the VillanovaApp custom resource, see the [External DB](../../tutorials/devops/external-db.md) document for more information.  
...
Villanova can auto-create a new Keycloak instance, but previous Secrets, clients, and permissions will be replaced and recreated from scratch. A less disruptive path is to backup and restore the VillanovaApp’s realm to the new Keycloak instance so there is no data loss. For detailed instructions, follow the [Backing Up and Restoring Keycloak](../../tutorials/devops/backing-restoring-keycloak.md) tutorial. If only the VillanovaApp is restored and the original Keycloak instance is used, it does not need to be reconfigured.
...
1. Export all the VillanovaDeBundle custom resources from the source instance and restore them to the new instance, 
...
3. For bundle DB schemas, different methods are required for auto-provisioned and external databases.
    ...
    A. For auto-provisiond DBMS schemas:  
             ...
             2. In case of Villanova Blueprint-generated Spring Boot plugins, the plugin's application will autonomously apply the Liquibase data migration scripts.   
    ...
    B. For external DBMS schemas:  
       1. Export all the VillanovaDeBundle CRs from the source instance and restore them to the target instance.  
...
There are different ways to resolve this issue, but the simplest method is to edit the ingress host name in the Villanova App Engine custom resource, `VillanovaApp`, using kubectl or the container orchestration tool of your choice. You will need to be the super admin or root user to make this change.
