...
This tutorial describes how to backup an active Villanova Identity Management Keycloak instance and restore it in another environment.

## Prerequisites
A Keycloak instance running on Villanova with a database management system (DBMS) in the backend. This procedure will not work with an embedded database. 
...
4. Run the following script to retrieve the data for the Villanova realm and save it to a JSON file in the `export` directory. A new Keycloak server will run on a different port (offset=200) to avoid conflicts with the original Villanova Keycloak instance.
``` bash
...
1. Login to the new Villanova Keycloak server where the realm is to be restored.

2. From the left sidebar, go to `Import`.
 
3. Click `Select file` button and `upload` the JSON file. You will see the following page:

![Villanova ID Management UI](./img/import-keycloak.png)
