...
7. Verify that you configured the `ingress.class` in the Operator `ConfigMap` so Villanova knows which ingress controller should be used:
...
There are situations where the default NGINX ingress configuration isn't optimized for Villanova, e.g., JWT tokens can be too large or `proxy-buffer-size` can be too small. A `502 Bad Gateway` error may indicate that the config needs to be modified.
