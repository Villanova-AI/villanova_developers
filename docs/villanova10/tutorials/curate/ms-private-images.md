...
## Tutorial
The first step demonstrates how to create a Secret for Docker Hub which is then added to the Villanova Operator ConfigMap. See the [corresponding Kubernetes documentation](https://kubernetes.io/docs/tasks/configure-pod-container/pull-image-private-registry) for other options.
...
* The Villanova namespace, e.g. `Villanova` for a quickstart environment
...
**2a. Deploy to a new Villanova Application**

If you're setting up a new Villanova Application, you can [add the Secret to the Villanova Operator ConfigMap](../consume/entando-operator.md) under the property `entando.k8s.operator.image.pull.secrets`. This is a list containing the names of Docker Secrets in the Operator's namespace.
...
**2b. Update an existing Villanova Application**

If you're updating an existing Villanova Application, you can add the new Secret to the `entando-plugin` serviceaccount.
...
**3. Install the Villanova Bundle**

 You can now install Villanova Bundles from the `Entando App Builder` → `Hub`. The microservice plugin should be able to successfully pull the image.
