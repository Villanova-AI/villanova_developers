...
An Villanova Content Delivery Server (CDS) is required in order to enable multiple tenants to be served by the same Villanova Application. This tutorial describes the steps required to setup CDS and configure the Villanova App Engine to use it.
...
<EntandoCode>curl -sLO "https://github.com/Villanova-AI/villanova-releases"</EntandoCode>
...
* In order to enable TLS, add a TLS secret and configure it on the ingress. Note that public URLs (e.g., `CDS_PUBLIC_URL`) should use the same **http** or **https** protocol as the Villanova Application. Private/cluster-level URLs (e.g., `CDS_PRIVATE_URL`) should use **http**. 
...
## Configure the VillanovaApp to use CDS
::: tip
The Villanova App Engine needs to be reconfigured just for the initial or primary tenant so all tenants use CDS in the same way.
:::

1. Scale the VillanovaApp deployment down to 0 replicas:
``` bash
