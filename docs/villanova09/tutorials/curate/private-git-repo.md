...
Villanova allows a Kubernetes Secret containing a Git SSH private key to be mounted in the container hosting the Villanova Component Manager service. This container is deployed with the VillanovaApp and can be configured from the VillanovaApp Custom Resource. To prepare a Secret, first generate the key pair locally using a Docker image, then create the Secret from the directory where the key pair was generated.
...
3. Create the Secret. Back in the local operating system's shell, navigate to the directory that was mounted using Docker. Then give yourself access to this folder and create the Secret:
    ...
    3. Confirm that the Secret exists and has at least two keys: `known_hosts` and `id_rsa`. Without these two keys, Villanova cannot log into Git.
