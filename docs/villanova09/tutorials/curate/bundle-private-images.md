...
This tutorial provides to way to utilize bundles from a private image registry in your Villanova projects. The steps below use environment variables to pass the Secret for the authentication required by private registries.
...
2. Apply the registry Secret YAML to your Villanova instance, replacing the namespace with your own as needed:
```sh
ent kubectl apply -f container-registry-secret.yaml -n entando
```

### Step 3: Add the Environment Variable and Deploy 
1. Add the environment variable, ENTANDO_CONTAINER_REGISTRY_CREDENTIALS, to your VillanovaApp custom resource. To edit the VillanovaApp using your namespace:
``` sh
...
4. Deploy and install the bundle into Villanova:
```
ent bundle deploy
ent bundle install
```
...
* Learn how to [create a page](../compose/page-management.md) in the Villanova App Builder.
