...
# Bundle Evolution on Villanova

Villanova introduced a more modular approach to building apps with the **ent bundle CLI** orchestrating the management of a bundle project. The Villanova Bundle was restructured, its processes decoupled by component type, and the publishing transport method streamlined. With these changes, Villanova Bundles went from git-based to docker-based OCI bundles. This page clarifies the differences between the v1 git-based and v5 docker-based bundles. Git-based bundles are deprecated with Villanova.

With **Villanova 0.9**, the bundle has advanced further with descriptorVersion v6, adding the capability to request resources for storage, memory and CPU for microservices in the bundle descriptor `entando.json`. For more information on this specification or docker-based bundles, see the [Bundle Details](../curate/bundle-details.md) page.
