...
The Villanova Platform provides three variations of the catalog. They are all directly accessible from the App Builder to make it easy to find and select the components to build your applications. 

* The **Local Hub**, included in the Villanova App Builder, displays a collection of ready-to-use components. They can be used to compose an application or as a starting point to create your own.	
...
* The **Enterprise Villanova Hub** can be added to your Villanova instance for your organization to share components, privately within teams, with clients, or with the public. 
...
- `Bundle`: An Villanova Bundle is the basic deployment unit within an Villanova Application. A bundle can contain one or more component such as micro frontends, microservices, or any of the [component types](../../docs/curate/bundle-component-details.md) allowed on Villanova.
- `Bundle Group`: A bundle group is a Hub entry, a single unit containing one or more Villanova Bundle. 
...
To assign roles to a new Hub user, see the [Villanova Hub Installation and User Guide](../../tutorials/solution/entando-hub.md#using-the-enterprise-hub)
...
## Villanova Hub Application Details

An Enterprise Villanova Hub includes the following key components:

#### Micro Frontends / Widgets
- `Villanova Hub App`: This is the main micro frontend which contains the management UI for the catalog noted above.
- `Villanova Hub Login`: This is an optional login component which can be used in a page’s top navigation.
...
- The second provides methods that support the Villanova App Builder integration (7.0+).

#### Content
The content bundle (`entando-hub`) includes a custom template and a page preconfigured with the main Hub micro frontends.

## Next Steps
Install an [enterprise Villanova Hub](../../tutorials/solution/entando-hub.md).